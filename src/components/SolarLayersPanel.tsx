...
export function SolarLayersPanel({parts,enabled,active,onLesson,onActivities}:{parts:PhenomenonParts;enabled:boolean;active:boolean;onLesson:(id:SolarLesson)=>void;onActivities:()=>void}){
 ...
 <div className="solar-layer-steps" role="group" aria-label="太阳分层观察顺序">{SOLAR_LESSONS.map(s=><button key={s.id} disabled={!enabled} aria-pressed={enabled&&active&&matchesSolarLesson(parts,s.id)} style={{borderLeftColor:s.color}} onClick={()=>onLesson(s.id)}>{s.title}</button>)}</div>
 <p role="status">{!enabled?'太阳活动未显示；请开启阶段 07，并勾选本模块或点击上方“定位太阳活动”。':!active?'已保留分层设置；点击任一步骤定位太阳并观察。':current?`正在观察：${current.title}`:'自定义组合 · 下方可分别开关'}</p>
 {enabled&&active&&current&&<p className="solar-layer-reading">{current.text}</p>}
