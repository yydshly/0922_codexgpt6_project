...
export function EnceladusPanel({choices,enabled,ready,active,status,error,onRetry,onLocate,onParent,onChange}:{choices:EnceladusChoices;enabled:boolean;ready:boolean;active:boolean;status:string;error:string;onRetry:()=>void;onLocate:()=>void;onParent:()=>void;onChange:(patch:Partial<EnceladusChoices>)=>void}){
 ...
 <button disabled={enabled&&!ready} onClick={onLocate}>{enabled?'靠近土卫二':'开启阶段 04'}</button><p role="status">{status}</p>
 ...
 <button aria-pressed={!choices.cutaway} onClick={()=>{onChange({cutaway:false,jets:true});onLocate();}}>1 · 表面与喷流</button>{' '}
 <button aria-pressed={choices.cutaway} onClick={()=>{onChange({cutaway:true,ice:true,ocean:true,core:true});onLocate();}}>2 · 打开内部剖示</button>
 ...
 <button onClick={onParent}>3 · 返回土星系统</button>
 </fieldset>
 ...
 <p><strong>与周围的关系：</strong>部分喷出物补给土星稀薄的 E 环；它不同于明亮的 A/B/C 主环。本轮说明这一联系，没有计算逐粒子飞向环的路径。</p>
