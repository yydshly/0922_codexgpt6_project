...
import occultationReport from '../../public/data/events/io-occultation-2026-01-12.json';
...
export function MotionLessonPanel({occultationView,onOccultationView,coorbitalBatch,coorbitalEnabled,coorbitalError,onRetryCoorbital,trackYear,trackCount,trackError,onRetryTracks,familyStates,familyError,onRetryFamily,frame,phasesEnabled,current,epoch,now,time,enabled,onSpeed,onLesson,onExit}:{occultationView:OccultationView;onOccultationView:(view:OccultationView)=>void;coorbitalBatch:StateBatch|null;coorbitalEnabled:boolean;coorbitalError:string;onRetryCoorbital:()=>void;trackYear:number|null;trackCount:number;trackError:string;onRetryTracks:()=>void;familyStates:readonly MacroMoon[];familyError:string;onRetryFamily:()=>void;frame:StateFrame|null;phasesEnabled:boolean;current:MotionLesson|null;epoch:number|null;now:number|null;time:MacroTimeControls;enabled:boolean;onSpeed:(speed:number)=>void;onLesson:(id:MotionLessonId)=>void;onExit:()=>void}){
 ...
 const playbackBlock=motionPlaybackBlock({...time,now,enabled});
 ...
 return <section className="environment-journey" data-motion-lessons aria-label="太阳系运动与天象课程"><h3>运动与天象：从基础运动到观测案例</h3><p>共 14 节：基础运动与受照 → 锁定与共振 → 共轨参照 → 月食、日食与遮掩。先理解运动，再理解观察方向如何改变所见现象。连续播放与单步对照共用观测日期，整幅太阳系同步更新。选择课程后已准备好推荐速度，点击「连续播放」即可观察。关闭所需阶段或卫星图层会退出课程并暂停；重新开启后需重新选课。</p><h4>基础运动与受照</h4><ol>{MOTION_LESSONS.slice(0,5).map((s,i)=><li key={s.id}><button disabled={!ready||unavailable(s.id)} aria-pressed={s.id===current?.id} onClick={()=>onLesson(s.id)}>{i+1} · {s.name}</button></li>)}</ol>
 ...
 <p>当前提供机制说明与历表案例；潮汐演化、共振角及长期稳定性求解尚未实现。</p></div><div data-coorbital-module><h3>共轨与准卫星 · 同一运动，两种参照</h3><p>先看日心轨迹，再看随地球公转旋转的局部路径。切换视角不会改变日期。</p><div className="environment-controls">{MOTION_LESSONS.filter(s=>isCoorbital(s.id)).map(s=><button key={s.id} disabled={!ready||unavailable(s.id)} aria-pressed={current?.id===s.id} onClick={()=>onLesson(s.id)}>{s.name}</button>)}</div>{!coorbitalEnabled?<p>需要开启阶段 03 区域成员。</p>:!coorbitalReady&&<p role="status">{coorbitalError||'正在读取当前日期的准卫星历表…'}{coorbitalError&&<button onClick={onRetryCoorbital}>重试准卫星历表</button>}</p>}<p>全景中也有当前位置标记，可点击进入；阶段 03 与行星区域图层控制显示。它是准卫星案例，不是新增一颗地球卫星。</p></div><div data-eclipse-module><h3>天象案例 · 月食、日食与遮掩</h3><button disabled={!ready} aria-pressed={current?.id==='lunar-eclipse'} onClick={()=>onLesson('lunar-eclipse')}>进入 2026-03-03 月全食</button><button disabled={!ready} aria-pressed={current?.id==='solar-eclipse'} onClick={()=>onLesson('solar-eclipse')}>进入 2026-08-12 日全食</button><button disabled={!ready} aria-pressed={isOccultation} onClick={()=>onLesson('io-occultation')}>进入木卫一遮掩案例</button><p>会暂停并切到案例日期。月食看月球进入地影，日食看月影落到地球。另可观察木星掩木卫一。三例均有来源和偏差记录；不是城市可见性预报。</p></div>{!ready&&<p role="status">{!enabled?'需开启阶段 01 宏观结构。':time.error?'历表读取失败，请从时间栏重试。':time.loading?'正在读取所选日期历表…':'需要真实太阳系历表。'}</p>}{current&&<><h4 data-motion-current tabIndex={-1}>{current.title}</h4><p data-motion-route>第 {index+1} / {MOTION_LESSONS.length} 节 · {index<5?'基础运动与受照':index<9?'锁定与共振':index<11?'共轨参照':'天象案例'}<br/>{index<MOTION_LESSONS.length-1?`下一节：${MOTION_LESSONS[index+1].name}`:'已到本组末节，可返回全景或重选课程。'}</p>{isOccultation&&<div data-occultation-views="panel"><div className="environment-controls"><button aria-pressed={occultationView==='space'} onClick={()=>onOccultationView('space')}>三维空间</button><button aria-pressed={occultationView==='earth'} onClick={()=>onOccultationView('earth')}>地球所见</button></div><p>{occultationView==='space'?'可拖动旋转，查看前后关系；青色箭头指向画面外的地球。':'固定地心视线，看木卫一如何被遮住；可切回三维空间旋转。'} 切换保留日期和倍率。</p></div>}
 {isOccultation&&<div data-occultation-jumps><h4>观察消失与重现 · 模型接触时刻</h4><div className="environment-controls">{([['接近木星',occultationReport.model.contacts.first-600],['开始遮住',occultationReport.model.contacts.first+1],['完全遮住',occultationReport.model.contacts.second+2],['遮掩中段',(occultationReport.model.contacts.second+occultationReport.model.contacts.third)/2],['开始露出',occultationReport.model.contacts.third+2],['完全露出',occultationReport.model.contacts.fourth+2]] as const).map(([label,t])=><button key={label} disabled={!ready} onClick={()=>{pause();time.onSeek(t);}}>{label}</button>)}</div><p>这些按钮按模型几何接触定位，允许与独立资料时刻不同；“露出”不等于当地实际可见。</p></div>}
 {!isTheory&&<div className="motion-playback" data-motion-playback>
  <strong role="status">{time.error?'读取失败':time.loading?'正在载入历表，运动暂缓':isEclipse&&now!==null&&!inWindow(now)?'当前日期不在案例窗口':atEnd?(isEclipse?'已到案例末尾':'已到历表末尾'):time.playing?'正在连续运动':isOccultation&&occultationView==='earth'?'已暂停 · 固定视线，可滚轮缩放':'已暂停 · 可以拖动视角观察'}</strong>
  <div className="environment-controls">
   <button className="motion-play-button" disabled={!time.playing&&!!playbackBlock} title={playbackBlock||undefined} onClick={time.onToggle}>{time.playing?'暂停运动':'连续播放'}</button>
  ...
  <label>观察速度 <select aria-label="运动课程速度" value={time.speed} disabled={!ready} onChange={event=>onSpeed(Number(event.target.value))}>
   ...
   {speedOptions.map((value,i)=><option key={value} value={value}>{i===0?motionSpeedLabel(value):`${['实时','慢速','推荐','快速'][i]} · ${motionSpeedLabel(value)}`} </option>)}
  </select></label>
  <p data-motion-duration>当前 {motionSpeedLabel(time.speed)}；{isEclipse?'完整 8 小时案例窗口':coorbital?'365.25 日对照时段（不是精确周期）':isJupiter?'一个木卫三平均绕木星周期':resonance==='moon'?'一个平均绕地周期':resonance==='mercury'?'两个平均绕日周期':isPhase?'一个平均月相循环':isOrbit?'一个公转周期':'自转一圈'}约需屏幕时间 {durationLabel}（不含载入等待）。</p>
  <small>速度只改变观测时间推进，不改变天体自身周期。金星和地球课程的倍率不同，不能按屏幕转速比较真实快慢。到历表或本节案例窗口末尾自动暂停，不循环回放。</small>
  {atEnd&&<p>可点击下方「重置本节日期」返回起点，再连续播放。</p>}
 </div>
 }
 {isEclipse&&<div data-eclipse-jumps><h4>案例时刻 · 北京时间</h4><p>{eclipseBeijing(event!.reference)} 为资料{isOccultation?'掩始':'食甚'}换算时刻。</p><div className="environment-controls">{[-4,-2,-1,0,1,2,4].map(hours=><button key={hours} disabled={!ready} onClick={()=>{pause();time.onSeek(event!.reference+hours*3600);}}>{hours===0?(isOccultation?'资料掩始':'资料食甚'):`${hours>0?'+':''}${hours} 小时`}</button>)}</div><button disabled={!ready} onClick={()=>{pause();time.onSeek(event!.start);}}>从案例开始重新观察</button></div>}
 ...
 <p>{current.text}</p>{coorbital&&<p data-coorbital-tracks>{trackYear} 年轨迹 · {trackCount?`${trackCount} 个同日采样点 / 天体`:trackError||'正在读取轨迹…'}{trackError&&<button onClick={onRetryTracks}>重试全年轨迹</button>}</p>}<p className="environment-legend">{current.note}</p>{isEclipse?(isOccultation?<OccultationReadout view={occultationView} time={now}/>:current.id==='solar-eclipse'?<SolarEclipseReadout frame={frame}/>:<LunarEclipseReadout frame={frame}/>):coorbital?<CoorbitalReadout mode={coorbital} batch={coorbitalBatch} now={now}/>:isTheory?<TidalMechanism key={current.id}/>:isJupiter?<OrbitalResonanceReadout now={now} epoch={epoch} states={familyStates}/>:resonance?<SpinOrbitReadout body={resonance} now={now} epoch={epoch}/>:isSeason?<SeasonReadout frame={frame}/>:isPhase?<><MoonPhaseReadout frame={frame}/><p>绕地球一周约 27.3 天；从一次新月到下一次平均约 29.5 天。后者仅用于估计观看时长，画面位置仍由历表决定。</p></>:<p>本体模型恒星自转周期：{(spinPeriodSeconds(current.body)/3600).toLocaleString('zh-CN',{maximumFractionDigits:3})} 小时；资料公转周期：{bodyById[current.body].orbitalPeriodDays.toLocaleString('zh-CN',{maximumFractionDigits:3})} 日。</p>}{!isTheory&&<><h4>单步对照 · 暂停并跳转日期</h4><div className="environment-controls"><button disabled={!can(-delta)} onClick={()=>move(-delta)}>{isOccultation?'后退 30 秒':isEclipse?'后退 15 分钟':coorbital?'后退 30 天':isJupiter?'后退 ¼ 木卫三周期':resonance==='moon'?'后退 ¼ 平均绕地周期':resonance==='mercury'?'后退 ½ 平均绕日周期':isPhase?'后退 3 天':isOrbit?'后退 30 天':'后退 ¼ 自转周期'}</button><button disabled={!can(delta)} onClick={()=>move(delta)}>{isOccultation?'前进 30 秒':isEclipse?'前进 15 分钟':coorbital?'前进 30 天':isJupiter?'前进 ¼ 木卫三周期':resonance==='moon'?'前进 ¼ 平均绕地周期':resonance==='mercury'?'前进 ½ 平均绕日周期':isPhase?'前进 3 天':isOrbit?'前进 30 天':'前进 ¼ 自转周期'}</button><button disabled={!ready||epoch===null||epoch<time.start||epoch>time.end} onClick={()=>{if(epoch!==null){if(time.playing)time.onToggle();time.onSeek(epoch);}}}>重置本节日期</button></div><p>每次步进 {Math.round(delta/3600*100)/100} 小时；越出内置历表或本节案例窗口时禁用。单步直接显示新日期，不播放中间过程；连续运动请使用上方播放按钮。课程与底部倍率同步，快进自转可能产生采样错觉。退出会暂停，保留当前日期和倍率；需要返回起点时先重置本节日期。</p></>}<div className="environment-controls"><button disabled={!ready||index===0||unavailable(MOTION_LESSONS[index-1]?.id)} onClick={()=>onLesson(MOTION_LESSONS[index-1].id)}>上一节运动</button><button disabled={!ready||index===MOTION_LESSONS.length-1||unavailable(MOTION_LESSONS[index+1]?.id)} onClick={()=>onLesson(MOTION_LESSONS[index+1].id)}>下一节运动</button></div><button onClick={()=>{pause();onExit();}}>退出运动课并恢复全景</button><p><a href={current.source} target="_blank" rel="noreferrer">课程来源 / 独立资料 ↗</a> · <a href={PHYSICAL_SOURCE} target="_blank" rel="noreferrer">JPL 参数 ↗</a> · <a href="https://naif.jpl.nasa.gov/pub/naif/generic_kernels/pck/pck00011.tpc" target="_blank" rel="noreferrer">NAIF 自转常数 ↗</a></p>{isEclipse?<small>本例仅作空间几何核对；地方可见性、天气与实际颜色没有计算。报告中的差值不是插值误差。</small>:coorbital?<small>当前两年位置与采样路径来自 Horizons；不据此求解共轨捕获史、长期稳定性或未来撞击风险。</small>:isTheory||isJupiter?<small>来源分别支持形成机制和已知轨道共振关系；原理图不对应当天形变，周期对照不代替独立共振动力学验证。</small>:resonance?<small>月球同步自转与水星 3:2 是两种周期关系，不代表本课已经模拟其形成过程。<a href={RESONANCE_SOURCE} target="_blank" rel="noreferrer">水星 3:2 共振原始研究 ↗</a></small>:isSeason?<small>两半球对照采用既有地轴姿态近似与同日历表；四个日期用于观察变化，未求解精确分至点。<a href="https://www.weather.gov/fgz/Seasons" target="_blank" rel="noreferrer">NWS：理想昼长与实测日出日落的区别 ↗</a></small>:isPhase?<small>主画面的月球亮面取决于当前自由镜头，旁边的月相圆面固定地心参照，所以二者不必相同。太阳在局部画面外，箭头只示传播方向；地月距离压缩，不能用此图判断食的几何条件。</small>:<small>金色线和粉色标记为教学标注，标记不是地理点。自转使用原有 NAIF 线性角速度与静态极轴近似，绝对经度/姿态不用于测绘；本体周期由该角速度反算，可能与四舍五入的参数表略有差别。</small>}</>}{!current&&<p>选择课程同步定位；原有「行星运动 · 谁转得快」仍提供所有行星的参数比较。</p>}</section>;
}
