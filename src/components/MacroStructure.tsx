...
interface IntegratedOptions {binary:BinaryOptions;families:FamilySceneOptions;flags:IntegratedFlags;target:IntegratedTarget|null;request:number;restore:number;progress:number;onFocus:(target:IntegratedTarget)=>void}
...
function MacroCanvas({ integrated, selectedMember, onSelectMember, focusRequest, restoreRequest, cometBatch, cometTracks, showActivity, frame, selected, resetCount, family, cameraView, heightScale, showPlane, layers, batch, animate, showLabels, onDistance }: { integrated:IntegratedOptions; selectedMember:string|null; onSelectMember:(id:string)=>void; focusRequest:number; restoreRequest:number; cometBatch: StateBatch | null; cometTracks: CometTracks | null; showActivity: boolean; frame: StateFrame | null; selected: MacroZoneId; resetCount: number; family?: SolarFamilyId; cameraView: MacroCameraView; heightScale: 1 | 10; showPlane: boolean; layers: MacroLayerVisibility; batch: StateBatch | null; animate: boolean; showLabels: boolean; onDistance: (distance: number) => void }) {
  ...
  useEffect(() => {
    ...
    const draw = (now: number) => {
      ...
      binaryScene.update(integration.binary,camera,integration.target);
      const followsBody=isMacroFamily(integration.target)||integration.target==='pluto-system';
      const followingPosition=integration.target==='pluto-system'?(integration.binary.state?binaryScene.anchor:null):current&&isMacroFamily(integration.target)?parentPositions[integration.target]:null;
      familyScene.update(current,{...integration.families,enabled:integration.families.enabled&&integration.target!=='pluto-system'},camera,isMacroFamily(integration.target)?integration.target:null,parentPositions);
      ...
      if(integration.target&&integration.request!==lastIntegratedRequest&&(!followsBody||followingPosition)){
        ...
        const target=integration.target==='pluto-system'?binaryScene.anchor:isMacroFamily(integration.target)?familyScene.anchors[integration.target]:integratedScene.anchors[integration.target];
        controls.enableDamping=false;controls.update();controls.minDistance=.4;camera.up.set(0,1,0);
        const members=integration.target==='pluto-system'&&integration.binary.state?[{position:integration.binary.state.relative,velocity:integration.binary.state.velocity}]:isMacroFamily(integration.target)?integration.families.states.filter(s=>s.parentId===integration.target):[];
    ...
    <div className="macro-main">
      ...
      <aside className="macro-info">
        ...
        {scope==='solar'&&<section className="panorama-integration" hidden={panelTab!=='integrated'} aria-label="全景现象控制">
          ...
          {integratedTarget&&<div className="panorama-current"><strong>当前定位：{(integratedTarget==='pluto-system'?'冥王星—卡戎':isMacroFamily(integratedTarget)?bodyById[integratedTarget].name+'系统':({sun:'太阳活动',dust:'行星际碎屑',helio:'日球层环境'}[integratedTarget]))}</strong><button onClick={leaveIntegrated}>返回定位前视角</button></div>}
