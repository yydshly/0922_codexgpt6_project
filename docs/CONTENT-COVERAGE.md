...
待核对与边界：F01 实现差距已修复并完成方向与交互检查；固定历元、不提供主全景恒星距离，不等于当前夜空或用户验收通过。

表示依据：Hipparcos 固定历元星表与筛选距离样本；主全景星点另为装饰。入口操作：恒星系统与其他星系 → 邻近恒星；切换天空方向/空间距离并点选资料。主全景底部“星表背景”可查看出处，也可点选星点查询 HIP 编号。
