import {PanoramaStarPanel} from './PanoramaStarPanel';
...
import { TextureLoadStatus } from './TextureLoadStatus';
import {solarObservationKey} from '../data/observationHistory';
import {MotionQuickControls} from './MotionQuickControls';
...
import type {OccultationView} from '../data/occultation';
import {createOccultationScene} from './occultationScene';
import {OccultationReadout} from './OccultationReadout';
import {createSolarEclipseScene} from './solarEclipseScene';
import {SolarEclipseReadout} from './SolarEclipseReadout';
import {eclipseCase} from '../data/eclipseCases';
import {createLunarEclipseScene} from './lunarEclipseScene';
import {LunarEclipseReadout} from './LunarEclipseReadout';
import {createCoorbitalScene} from './coorbitalScene';
import {CoorbitalReadout} from './CoorbitalReadout';
import {coorbitalProvider,coorbitalState,isCoorbital} from '../data/coorbital';
import {useCoorbitalTracks} from '../hooks/useCoorbitalTracks';
import {resonantStates,resonanceReady} from '../data/tidalModule';
import {OrbitalResonanceReadout} from './OrbitalResonanceReadout';
import {createMoonLockScene} from './moonLockScene';
import {SpinOrbitReadout} from './SpinOrbitReadout';
import {needsMoonFamily,spinOrbitBody} from '../data/spinOrbit';
import {createSeasonScene} from './seasonScene';
import {SeasonReadout} from './SeasonReadout';
import {earthSeasonPole} from '../data/seasons';
import {createMoonPhaseScene} from './moonPhaseScene';
import {MoonPhaseReadout} from './MoonPhaseReadout';
import {MotionLessonPanel} from './MotionLessonPanel';
import {motionLessonForView,motionPlaybackSpeed,MOTION_LESSONS,type MotionLessonId} from '../data/motionLessons';
import {createMotionReference} from './motionReference';
import {BoundaryComparisonPanel} from './BoundaryComparisonPanel';
import {boundaryForView,boundaryLayers,type BoundaryId} from '../data/boundaryComparison';
...
import { CosmicCanvas,type CosmicCamera,type CosmicCameraBridge } from './CosmicCanvas';
import {StellarCataloguePanel,StellarViewNotice} from './StellarCataloguePanel';
import {useStarCatalogue} from '../hooks/useStarCatalogue';
import {STELLAR_GUIDE_HIP,canShowStarInSpace,type StellarView} from '../data/starCatalogue';
...
interface IntegratedOptions {textures:{retry:number;onStatus:(items:TextureLoadItem[])=>void};occultationView:OccultationView;coorbital:StateBatch|null;coorbitalTracks:StateBatch[];coorbitalEnabled:boolean;onCoorbital:()=>void;motionEpoch:number|null;motion:MotionLessonId|null;familyContext:boolean;intent:IntegratedId|null;erosShape:ErosShapeOptions;patroclus:PatroclusSceneOptions;eris:ErisSceneOptions;smallRings:SmallRingOptions;parts:PhenomenonParts;planetFocus:boolean;planetOrbits:PlanetOrbitOptions;earth:EarthAppearance&{retry:number;onStatus:(status:CloudStatus)=>void};comets:CometDisplay;binary:BinaryOptions;families:FamilySceneOptions;flags:IntegratedFlags;target:IntegratedTarget|null;request:number;restore:number;progress:number;onFocus:(target:IntegratedTarget,intent?:IntegratedId)=>void}

interface ObservationBookmark {
 scope:'solar'|'cosmic';cosmic:CosmicLevelId;stellarView:StellarView;selectedStar:number|null;cosmicCamera:CosmicCamera|null;
 topic:TopicSelection|null;
 occultationView:OccultationView;motion:MotionLessonId|null;motionEpoch:number|null;key:string;title:string;camera:CameraBookmark|null;
...
function MacroCanvas({ starCatalogue,starBackground,onStar, historical,onPresence,active, historyBridge, integrated, selectedMember, onSelectMember, focusRequest, restoreRequest, cometBatch, cometTracks, showActivity, frame, selected, resetCount, family, cameraView, heightScale, showPlane, layers, batch, animate, showLabels, onDistance }: { starCatalogue:StarCatalogue|null;starBackground:boolean;onStar:(hip:number)=>void;historical:HistoricalView|null;onPresence:(snapshot:SceneSnapshot)=>void;active:boolean; historyBridge:RefObject<CameraHistoryBridge>; integrated:IntegratedOptions; selectedMember:string|null; onSelectMember:(id:string)=>void; focusRequest:number; restoreRequest:number; cometBatch: StateBatch | null; cometTracks: CometTracks | null; showActivity: boolean; frame: StateFrame | null; selected: MacroZoneId; resetCount: number; family?: SolarFamilyId; cameraView: MacroCameraView; heightScale: 1 | 10; showPlane: boolean; layers: MacroLayerVisibility; batch: StateBatch | null; animate: boolean; showLabels: boolean; onDistance: (distance: number) => void }) {
  ...
  const displayRef = useRef({ starCatalogue,starBackground,onStar,historical,onPresence,integrated, selectedMember, onSelectMember, focusRequest, restoreRequest, cometBatch, cometTracks, showActivity, layers, batch, animate, showPlane, heightScale, family, showLabels, onDistance });
  displayRef.current = { starCatalogue,starBackground,onStar,historical,onPresence,integrated, selectedMember, onSelectMember, focusRequest, restoreRequest, cometBatch, cometTracks, showActivity, layers, batch, animate, showPlane, heightScale, family, showLabels, onDistance };
  ...
  useLayoutEffect(() => {
    ...
    const skyBackground=createPanoramaStars(scene,particleTexture);
    ...
    const motionReferences={earth:createMotionReference(primaryMeshes.earth),venus:createMotionReference(primaryMeshes.venus),mercury:createMotionReference(primaryMeshes.mercury)};
    motionReferences.mercury.add(new THREE.ArrowHelper(new THREE.Vector3(1,0,0),new THREE.Vector3(),.25,'#ffa7cf',.045,.025));
    ...
    const moonPhaseScene=createMoonPhaseScene(scene);
    const moonLockScene=createMoonLockScene(scene);
    const lunarEclipseScene=createLunarEclipseScene(scene,element);
    const solarEclipseScene=createSolarEclipseScene(scene,element);
    const occultationScene=createOccultationScene(scene,element);
    const coorbitalScene=createCoorbitalScene(scene,element,()=>displayRef.current.integrated.onCoorbital());
    const seasonScene=createSeasonScene(scene);
    const familyScene=createMacroFamilies(scene,element,id=>displayRef.current.integrated.families.onSelect(id),id=>displayRef.current.integrated.families.onFocus(id),textureLoader);
    ...
    const pointerUp=(event:PointerEvent)=>{
      ...
      scene.traverse(object=>{if(!object.userData.memberId&&!object.userData.familyMoon&&!object.userData.binaryId&&!(object.userData.coorbital&&!isCoorbital(displayRef.current.integrated.motion)))return;let p:THREE.Object3D|null=object;while(p){if(!p.visible)return;p=p.parent;}markers.push(object);});
      const hit=raycaster.intersectObjects([...markers,...planets.filter(p=>p.visible),...(sun.visible?[sun]:[])],false)[0];
      if(hit?.object.userData.coorbital)displayRef.current.integrated.onCoorbital();
      else if(hit?.object.userData.binaryId)displayRef.current.integrated.binary.onSelect(hit.object.userData.binaryId);
      ...
      else if(!hit){const hip=skyBackground.pick(camera,event.clientX-bounds.left,event.clientY-bounds.top,bounds.width,bounds.height);if(hip!==null)displayRef.current.onStar(hip);}
    };
    ...
    const diagnostics=sceneDiagnostics(renderer,'solar-panorama');
    const draw = (now: number) => {
      animation = requestAnimationFrame(draw);
      if(!activeRef.current){lastFrame=now;return;}
      // Use display refresh; orbital state and demonstration time use elapsed time, not frame count.
      ...
      if(history){
        ...
        controls.update();historicalGeometry?.update(history.time,camera,element.clientWidth,element.clientHeight);renderer.render(historicalScene,camera);diagnostics.frame();return;
      }
      ...
      const motion=integration.motion;const lunarEclipse=motion==='lunar-eclipse',solarEclipse=motion==='solar-eclipse',occultation=motion==='io-occultation';const spatialOccultation=occultation&&integration.occultationView==='space';controls.enableRotate=!occultation||spatialOccultation;controls.enablePan=!occultation||spatialOccultation;controls.maxDistance=occultation&&!spatialOccultation?500:95;occultationScene.update(current?.time??null,occultation,integration.occultationView);const coorbital=isCoorbital(motion)?motion:null;const orbitalLesson=motion==='earth-orbit'||motion==='mercury-resonance';const orbitalBody=motion==='mercury-resonance'?'mercury':'earth';
      const primary=primaryId(integration.target);
      motionReferences.earth.visible=motion==='earth-spin';motionReferences.venus.visible=motion==='venus-spin';motionReferences.mercury.visible=motion==='mercury-resonance';
      ...
      const isolatedPrimary=orbitalLesson?null:primary??(isMacroFamily(integration.target)?integration.target:null);
      ...
      if(orbitalLesson)planets.forEach(mesh=>mesh.visible=!!current&&mesh.userData.primaryId===orbitalBody);
      if(isolatedPrimary){plane.visible=false;guides.forEach(g=>{g.visible=false;});}
      if(textureRetry!==integration.textures.retry){textureRetry=integration.textures.retry;textureLoader.retryFailed();}
      ...
      moonPhaseScene.update(current,motion==='moon-phase',planets[2].position);
      moonLockScene.update(current,motion==='moon-lock',planets[2].position);
      seasonScene.update(current,motion==='earth-seasons',planets[2].position);
      ...
      planetOrbits.update(current,orbitalLesson?{orbits:true,scales:false,direction:true}:integration.planetOrbits,displayRef.current.layers.planetary,orbitalLesson?orbitalBody:orbitFocus,parentPositions,heightScale,orbitalLesson?orbitalBody:undefined);
      ...
      const followingPosition=occultation?occultationScene.anchor.clone():(coorbital||lunarEclipse||solarEclipse)?new THREE.Vector3():enceladusRequested?enceladusCenter:primary?(current?primaryMeshes[primary].position:null):isCometId(integration.target)?(cometPoint?new THREE.Vector3(...cometPoint):null):integration.target==='pluto-system'?(integration.binary.state?binaryScene.anchor:null):current&&isMacroFamily(integration.target)?parentPositions[integration.target]:null;
      familyScene.update(current,{...integration.families,states:motion==='jupiter-resonance'?resonantStates(integration.families.states):material?.id==='e-ring'?integration.families.states.filter(m=>m.id==='enceladus'):integration.families.states,isolate:!!isolatedFamily,moons:integration.families.moons&&(!primary||integration.familyContext),enabled:integration.families.enabled},camera,isMacroFamily(primary)?primary:isMacroFamily(integration.target)?integration.target:null,parentPositions);
      ...
      if(integration.target&&integration.request!==lastIntegratedRequest&&(!followsBody||followingPosition)){
        ...
        if(motion==='earth-spin'||motion==='venus-spin')direction.set(1,.35,.4).normalize().applyQuaternion(primaryMeshes[motion==='earth-spin'?'earth':'venus'].quaternion);
        if(motion==='earth-seasons'){const toSun=target.clone().negate().normalize();direction.crossVectors(new THREE.Vector3(0,1,0),earthSeasonPole()).normalize();if(direction.dot(toSun)<0)direction.negate();}
        if(motion==='jupiter-resonance')direction.copy(satelliteOverviewDirection(resonantStates(integration.families.states),target.clone().negate()));
        if(lunarEclipse||occultation)direction.set(0,0,1);if(spatialOccultation)direction.set(1,.5,.8).normalize();
        if(solarEclipse)direction.set(0,0,-1);
        if(coorbital)direction.set(.1,1,.55).normalize();
      ...
      if(lunarEclipse||solarEclipse||occultation){for(const object of scene.children){if(object instanceof THREE.Light||object.userData.background||object===(occultation?occultationScene.root:lunarEclipse?lunarEclipseScene.root:solarEclipseScene.root))continue;familyHidden.set(object,object.visible);object.visible=false;}}
      else if(coorbital){for(const object of scene.children){if(object instanceof THREE.Light||object.userData.background||object===coorbitalScene.root)continue;familyHidden.set(object,object.visible);object.visible=false;}}
      else if(orbitalLesson)isolateEnvironmentContext(scene,orbitalBody==='mercury'?'sun-mercury':'sun-earth',familyHidden,true);
      else if(medium)isolateEnvironmentContext(scene,medium.id==='chargedParticles'?'none':'sun',familyHidden);
      ...
      lunarEclipseScene.update(current,lunarEclipse);solarEclipseScene.update(current,solarEclipse);occultationScene.update(current?.time??null,occultation,integration.occultationView);
      coorbitalScene.update(integration.coorbital,current?.time,coorbital,integration.coorbitalTracks,integration.coorbitalEnabled&&displayRef.current.layers.planetary&&!integration.target&&!focus.selectedMember&&!family);
      ...
      skyBackground.update(displayRef.current.starCatalogue,camera,displayRef.current.starBackground&&!integration.motion&&!medium);
      element.dataset.backgroundStars=String(skyBackground.root.visible?displayRef.current.starCatalogue?.sky.length??0:0);
      ...
      lunarEclipseScene.layout(camera,element.clientWidth,element.clientHeight,showLabels);solarEclipseScene.layout(camera,element.clientWidth,element.clientHeight,showLabels);occultationScene.layout(camera,element.clientWidth,element.clientHeight,showLabels);
      coorbitalScene.layout(camera,element.clientWidth,element.clientHeight,showLabels);
      ...
      renderer.render(scene, camera);diagnostics.frame();
    };
    animation = requestAnimationFrame(draw);
    return () => {
      ...
      disposed=true;textureLoader.dispose();cancelAnimationFrame(animation); observer.disconnect(); controls.dispose(); sceneRef.current = null;historyBridge.current.capture=null;historyBridge.current.pending=null;
      historicalGeometry?.dispose();skyBackground.dispose();earthEffects.dispose();disposeSprites(scene);
      ...
      occultationScene.dispose();solarEclipseScene.dispose();lunarEclipseScene.dispose();coorbitalScene.dispose();primaryLabels.dispose();phenomena.dispose();integratedScene.dispose();familyScene.dispose();binaryScene.dispose();
  ...
  return <div className="macro-canvas" ref={host} role="group" aria-label="可拖动旋转、滚轮缩放的太阳系宏观结构三维示意">{integrated.motion==='io-occultation'&&<OccultationReadout view={integrated.occultationView} compact time={frame?.time??null}/>} {integrated.motion==='solar-eclipse'&&<SolarEclipseReadout compact frame={frame}/>} {integrated.motion==='lunar-eclipse'&&<LunarEclipseReadout compact frame={frame}/>} {isCoorbital(integrated.motion)&&<CoorbitalReadout compact mode={integrated.motion} batch={integrated.coorbital} now={frame?.time??null}/>} {integrated.motion==='moon-phase'&&<MoonPhaseReadout compact frame={frame}/>}{integrated.motion==='earth-seasons'&&<SeasonReadout compact frame={frame}/>}{integrated.motion&&spinOrbitBody(integrated.motion)&&<SpinOrbitReadout compact body={spinOrbitBody(integrated.motion)!} now={frame?.time??null} epoch={integrated.motionEpoch}/>}{integrated.motion==='jupiter-resonance'&&<OrbitalResonanceReadout compact now={frame?.time??null} epoch={integrated.motionEpoch} states={integrated.families.states}/>}</div>;
}

export function MacroStructure({ active=true,onSpeedChange,onHomeTool,initialPanel,initialContentId,initialCosmic,returnLabel,stageFlags, onOpenStages, onOpenEnvironment, onOpenSolarActivity, onOpenDust, onOpenHeliosphere, initialZone, onOpenFamily, initialFamily, initialMemberId, timeControls, frame, displayDate, isEphemeris = true, onClose, onOpenReadingGuide, onObservePlanets, onExploreObject }: Props) {
  ...
  const [occultationView,setOccultationView]=useState<OccultationView>('space');
  ...
  const topicNavigating=useRef(false);
  ...
  const [textureItems,setTextureItems]=useState<TextureLoadItem[]>([]);
  ...
  const coorbitalData=useMonthlyStates(coorbitalProvider,scientificFrame?.time,isEphemeris&&stageFlags.structure&&stageFlags.members);
  const coorbitalTracks=useCoorbitalTracks(scientificFrame?.time,isEphemeris&&stageFlags.structure&&stageFlags.members&&isCoorbital(motionLesson));
  ...
  const stars=useStarCatalogue(true);
  const [starBackground,setStarBackground]=useState(true),[backgroundHip,setBackgroundHip]=useState<number|null>(null);
  const revealStarBackground=(hip?:number)=>{if(hip!==undefined)setBackgroundHip(hip);setPanelTab('sources');requestAnimationFrame(()=>{const panel=infoScroll.current?.querySelector<HTMLElement>('[data-panorama-stars]');panel?.scrollIntoView({block:'start'});if(panel){panel.tabIndex=-1;panel.focus({preventScroll:true});}});};
  const [stellarView,setStellarView]=useState<StellarView>('space');
  const [selectedStar,setSelectedStar]=useState<number|null>(null);
  const [stellarReset,setStellarReset]=useState(0);
  const changeStellarView=(view:StellarView)=>{if(view==='space'&&!canShowStarInSpace(stars.data,selectedStar))return;setStellarView(view);};
  const guideStellarView=(step:0|1|2)=>{if(!stars.data)return;setStellarView(step===2?'sky':'space');setSelectedStar(step===0?null:STELLAR_GUIDE_HIP);setStellarReset(n=>n+1);};
  ...
  useEffect(()=>{const key=`${selected}/${solarTab}/${familyId}/${cameraView}/${resetCount}`;if(previousRegion.current!==key){if(!restoringObservation.current&&!topicNavigating.current)setSelectedMember(null);previousRegion.current=key;}},[selected,solarTab,familyId,cameraView,resetCount]);
  ...
  useEffect(()=>{if(!restoringObservation.current&&!topicNavigating.current)setIntegratedTarget(null);},[selected,solarTab,familyId,cameraView,resetCount]);
  ...
  const boundaryStep=stageFlags.structure&&stageFlags.heliosphereExplorer?boundaryForView(selected,solarTab,integratedTarget,selectedMember,layers,integratedChoices):null;
  const chooseBoundary=(id:BoundaryId)=>{if(!stageFlags.structure||!stageFlags.heliosphereExplorer)return;setLearningIndex(null);setScope('solar');setIntegratedTarget(null);setFocusIntent(null);setIntegratedPlaying(false);setSelectedMember(null);setFamilySelected(null);setBinarySelected(null);setPlanetFocus(false);setFamilyContext(false);setSolarTab('zones');setSelected(id);setLayers(boundaryLayers(id));setIntegratedChoices({solar:false,environment:false,belts:false,dust:false,helio:false});setShowPlane(false);setHeightScale(1);setCameraView('oblique');setShowLabels(true);setResetCount(n=>n+1);setPanelTab('integrated');};
  const motionStep=stageFlags.structure&&(!isCoorbital(motionLesson)||stageFlags.members)&&(!needsMoonFamily(motionLesson)||stageFlags.families&&effectiveLayers.moons)?motionLessonForView(motionLesson,integratedTarget):null;
  useEffect(()=>{if(topic||!motionStep||panelTab!=='integrated')return;const handle=requestAnimationFrame(()=>{const root=infoScroll.current,target=root?.querySelector<HTMLElement>('[data-motion-current]');if(root&&target)root.scrollTo({top:root.scrollTop+target.getBoundingClientRect().top-root.getBoundingClientRect().top-12,behavior:'instant'});});return ()=>cancelAnimationFrame(handle);},[motionStep?.id,panelTab]);
  const chooseMotion=(id:MotionLessonId)=>{if(isCoorbital(id)&&(!stageFlags.members||!coorbitalState(coorbitalData.batch,scientificFrame?.time))||id==='jupiter-resonance'&&!resonanceReady(familyStates)||!stageFlags.structure||needsMoonFamily(id)&&!stageFlags.families||!scientificFrame||timeControls.loading||timeControls.error)return;const lesson=MOTION_LESSONS.find(l=>l.id===id)!;onSpeedChange(motionPlaybackSpeed(id));if(timeControls.playing)timeControls.onToggle();setIntegratedPlaying(false);setLearningIndex(null);setScope('solar');setHeightScale(1);setShowPlane(false);setFamilyContext(false);setLayers({...boundaryLayers('heliosphere'),heliosphere:false,moons:needsMoonFamily(id)});if(needsMoonFamily(id)){setFamilyOrbits(true);setFamilyRings(false);setFamilyEnhanced(false);}setIntegratedChoices({solar:false,environment:false,belts:false,dust:false,helio:false});focusIntegrated(lesson.target,null);setMotionLesson(id);if(id==='io-occultation')setOccultationView('space');const event=eclipseCase(id);if(event)timeControls.onSeek(event.entry);setMotionEpoch(event?event.entry:isCoorbital(id)&&isCoorbital(motionLesson)&&motionEpoch!==null?motionEpoch:scientificFrame.time);};
  const chooseOccultationView=(view:OccultationView)=>{setOccultationView(view);setIntegratedRequest(n=>n+1);};
  // Closing a prerequisite ends the lesson; reopening must not resume a hidden clock or stale camera.
  useEffect(()=>{
    if(!motionLesson)return;
    const blocked=!stageFlags.structure||isCoorbital(motionLesson)&&!stageFlags.members||needsMoonFamily(motionLesson)&&(!stageFlags.families||!effectiveLayers.moons);
    if(!blocked)return;
    if(timeControls.playing)timeControls.onToggle();
    setMotionLesson(null);setMotionEpoch(null);setIntegratedTarget(null);setFocusIntent(null);
    setIntegratedPlaying(false);setIntegratedRestore(n=>n+1);
  },[motionLesson,stageFlags.structure,stageFlags.members,stageFlags.families,effectiveLayers.moons,timeControls.playing]);
  const motionEvent=eclipseCase(motionStep?.id);
  useEffect(()=>{const event=eclipseCase(motionStep?.id);if(!event||!timeControls.playing||!scientificFrame)return;if(scientificFrame.time>=event.end){timeControls.onToggle();timeControls.onSeek(event.end);}else if(scientificFrame.time<event.start)timeControls.onToggle();},[motionStep?.id,scientificFrame?.time,timeControls.playing]);
  const observationTitle=(motionStep?.id==='io-occultation'?`${motionStep.title} · ${occultationView==='space'?'三维空间':'地球所见'}`:motionStep?.title)??boundaryStep?.title??mediumStep?.title??materialStep?.title??environmentStep?.title??(integratedTarget?(focusIntent?focusPhenomenonTitle(focusIntent,integratedTarget)??'空间现象':primaryId(integratedTarget)?`${bodyById[primaryId(integratedTarget)!].name}本体`:isMacroFamily(integratedTarget)?`${integratedTarget==='earth'?'地月系统':`${bodyById[integratedTarget].name}卫星系统`}${selectedMoon?` · ${selectedMoon.name}`:''}`:integratedTarget==='pluto-system'?(binarySmall?'冥王星卫星家族':'冥王星—卡戎'):isCometId(integratedTarget)?COMETS.find(c=>c.id===integratedTarget)!.name:integratedTarget==='comet-demo'?'彗尾活动示例':INTEGRATED_ITEMS.find(i=>i.target===integratedTarget)?.title??'空间现象'):selectedMember?regionMemberById(selectedMember)?.name??'区域成员':solarTab==='families'?SOLAR_FAMILIES.find(f=>f.id===familyId)!.name:MACRO_ZONES.find(z=>z.id===selected)?.name??'太阳系整体结构');
  const observationKey=scope==='cosmic'?JSON.stringify(['cosmic',cosmicId,stellarView,selectedStar,stellarReset]):solarObservationKey({target:integratedTarget,member:selectedMember,zone:selected,tab:solarTab,family:familyId,view:cameraView,reset:resetCount,familySelected,binarySelected,intent:focusIntent,environment:environmentStep?.id??null,material:materialStep?.id??null,medium:mediumStep?.id??null,boundary:boundaryStep?.id??null,motion:motionStep?.id??null,occultationView});
  ...
  const captureObservation=():ObservationBookmark=>({scope,cosmic:cosmicId,stellarView,selectedStar,cosmicCamera:scope==='cosmic'?cosmicHistory.current.capture?.()??null:null,topic,occultationView,motion:motionStep?.id??null,motionEpoch,key:observationKey,title:scope==='cosmic'?`${COSMIC_LEVELS.find(c=>c.id===cosmicId)!.name}${cosmicId==='neighbors'?stellarView==='space'?' · 空间距离':' · 天空方向':''}`:observationTitle,camera:scope==='solar'?(historyBridge.current.pending??historyBridge.current.capture?.()??null):null,progress:integratedProgress,familyContext,intent:focusIntent,target:integratedTarget,member:selectedMember,zone:selected,tab:solarTab,family:familyId,view:cameraView,height:heightScale,plane:showPlane,layers:{...layers},choices:{...integratedChoices},parts:{...phenomenonParts},smallRings:{...smallRingChoices},erosShape:{...erosShapeChoices},eris:{...erisChoices},patroclus:{...patroclusChoices},planetFocus,enceladus:{...enceladusChoices},familyRings,familyEnhanced,familyOrbits,binaryCenter,binaryOrbits,familySelected,binarySelected,binarySmall,binaryEnhanced,showActivity,panel:panelTab});
  const captureNavigation=()=>{if(!restoringObservation.current)capturedObservation.current=captureObservation();};
  useEffect(()=>{
    ...
    const timer=window.setTimeout(()=>{
      if(!restoringObservation.current&&capturedObservation.current&&capturedObservation.current.key!==observationKey){const previous=capturedObservation.current;setObservationHistory(h=>recordObservation(h,previous));capturedObservation.current=null;historyBridge.current.pending=null;}
  ...
  const observationBlocked=(entry:ObservationBookmark|undefined)=>{
    if(!entry)return '';
    if(entry.scope==='cosmic')return entry.cosmic==='neighbors'&&!stars.data?(stars.error||'等待恒星目录'):'';
    const target=entry.target;if(isCoorbital(entry.motion)&&!stageFlags.members)return '请开启阶段 03 区域成员';if(needsMoonFamily(entry.motion)&&!stageFlags.families)return '请开启阶段 04 卫星家族';if(entry.motion&&!stageFlags.structure)return '请开启阶段 01 运动课程';
    if(boundaryForView(entry.zone,entry.tab,entry.target,entry.member,entry.layers,entry.choices)&&(!stageFlags.structure||!stageFlags.heliosphereExplorer))return '请先开启阶段 01 与阶段 09';
  ...
  const traverseHistory=(direction:'back'|'forward')=>{
    ...
    historyBridge.current.pendingKey=solarObservationKey({target:entry.target,member:entry.member,zone:entry.zone,tab:entry.tab,family:entry.family,view:entry.view,reset:resetCount+1,familySelected:entry.familySelected,binarySelected:entry.binarySelected,intent:entry.intent,environment:environmentStepForView(entry.target,entry.intent,entry.parts)?.id??null,material:materialStepForView(entry.target,entry.intent,entry.parts,entry.familySelected,entry.enceladus)?.id??null,medium:mediumLessonForView(entry.target,entry.intent,entry.parts)?.id??null,boundary:boundaryForView(entry.zone,entry.tab,entry.target,entry.member,entry.layers,entry.choices)?.id??null,motion:motionLessonForView(entry.motion,entry.target)?.id??null,occultationView:entry.occultationView});
    setScope(entry.scope);setCosmicId(entry.cosmic);setStellarView(entry.stellarView);setSelectedStar(entry.selectedStar);cosmicHistory.current.pending=entry.cosmicCamera;setStellarReset(n=>n+1);
    setTopic(entry.topic);setOccultationView(entry.occultationView);setMotionLesson(entry.motion);setMotionEpoch(entry.motionEpoch);setIntegratedPlaying(false);setIntegratedProgress(entry.progress);
  ...
  const changeObservationAngle=(view:MacroCameraView)=>{
    ...
    if((integratedTarget||selectedMember)&&bookmark){
      ...
      historyBridge.current.pendingKey=solarObservationKey({target:integratedTarget,member:selectedMember,zone:selected,tab:solarTab,family:familyId,view,reset:resetCount+1,familySelected,binarySelected,intent:focusIntent,environment:environmentStep?.id??null,material:materialStep?.id??null,medium:mediumStep?.id??null,boundary:boundaryStep?.id??null,motion:motionStep?.id??null,occultationView});
      setResetCount(n=>n+1);
    }
    setCameraView(view);
  };
  ...
  const restorePanorama=()=>{setTopic(null);setMotionLesson(null);
    setScope('solar');setFocusIntent(null);setIntegratedPlaying(false);setIntegratedTarget(null);setSelectedMember(null);setFamilySelected(null);setBinarySelected(null);
  ...
  const goLearning=(index:number)=>{
    ...
    setTopic(null);setMotionLesson(null);
  ...
  const cosmicFacts: Record<CosmicLevelId, [string, string]> = {
    neighbors: ['光年 · 32 个筛选样本', '恒星属于银河系，不是太阳系成员'],
  ...
  const revealContent = (id: string) => {
    // Wait for the selected panel to mount, then focus its explanation without moving the page.
  ...
  const visitHomeContent=(row:CoverageItem)=>{
    ...
    if(row.id==='E04'){setFamilyRings(true);focusFamily('saturn');return;}
  ...
  const readyIds=new Set([...(scientificFrame?BODY_IDS:[]),...familyData.states.map(s=>s.id),...(memberBatch?.states.map(s=>s.id)??[]),...(cometData.batch?.states.map(s=>s.id)??[]),...plutoStates.map(s=>s.id),...(erisState&&stageFlags.families?['dysnomia']:[]),...(patroclusState&&stageFlags.families?['menoetius']:[]),...(binaryState?['charon']:[]),...(coorbitalState(coorbitalData.batch,scientificFrame?.time)?['kamo']:[])]);
  const memberLoader=(m:DirectoryMember)=>m.dataset.includes('/satellites/')?familyData:m.dataset.includes('/dwarfs/')?dwarfData:m.dataset.includes('/small-bodies/')?smallBodyData:m.dataset.includes('/comets/')?cometData:m.dataset.includes('/pluto-moons/')?plutoMoonsData:m.dataset.includes('/eris-system/')?erisData:m.dataset.includes('/patroclus-system/')?patroclusData:m.dataset.includes('/coorbital/')?coorbitalData:null;
  const memberStatus=(m:DirectoryMember):DirectoryStatus=>{
    if(m.kind==='history')return {text:'独立历史窗口 · 2019—2020，不计入当前就绪数',blocked:false,retry:false,stages:false};
    const missing=memberStageMissing(m,stageFlags);
    if(missing.length)return {text:`请开启 ${missing.map(id=>STAGES.find(s=>s.id===id)!.title).join('、')}`,blocked:true,retry:false,stages:true};
    if(!scientificFrame||timeControls.loading||timeControls.error)return {text:timeControls.error?'主历表读取失败，请从底部时间栏重试':'等待当前日期的主历表',blocked:true,retry:false,stages:false};
    const loader=memberLoader(m);
    if(loader?.error&&!readyIds.has(m.id))return {text:loader.error,blocked:true,retry:true,stages:false};
  ...
  const visitMember=(m:DirectoryMember)=>{
    ...
    const previous=captureObservation();
    if(m.kind!=='coorbital'&&timeControls.playing)timeControls.onToggle();
    setLearningIndex(null);setTopic(null);setMotionLesson(null);setIntegratedPlaying(false);setFocusIntent(null);setPlanetFocus(false);setFamilyContext(false);setHeightScale(1);setCameraView('oblique');setScope('solar');setSolarTab('zones');setPanelTab('integrated');
    setIntegratedChoices({solar:false,environment:false,belts:false,dust:false,helio:false});
    setLayers({...boundaryLayers('heliosphere'),heliosphere:false,moons:['moon','pluto'].includes(m.kind),dwarfs:['member','eris','patroclus'].includes(m.kind),comets:m.kind==='comet'});
    if(m.kind==='primary')focusIntegrated(primaryTarget(m.id as PrimaryId),null);
    else if(m.kind==='moon')selectFamilyMoon(m.id);
    else if(m.kind==='pluto'){setBinarySmall(true);focusBinary();setBinarySelected(m.id as PlutoSystemId);}
    else if(m.kind==='eris'){chooseMember('dysnomia');setErisChoices(v=>({...v,moon:true,selected:'dysnomia'}));}
    else if(m.kind==='patroclus'){chooseMember('menoetius');setPatroclusChoices(v=>({...v,moon:true,selected:'menoetius'}));}
    else if(m.kind==='comet')focusComet(m.id as CometId);
    else if(m.kind==='coorbital')chooseMotion('coorbital-sun');
    else chooseMember(m.id);
  ...
  const topicSatelliteError=(step:TopicStep)=>{
    if(step.stages.some(id=>!stageFlags[id]))return '';
    const a=step.action;
    const destination=a.kind==='learning'?LEARNING_STEPS.find(s=>s.id===a.id)?.destination:null;
    const parent=a.kind==='family'?a.id:a.kind==='detail'&&a.id==='enceladus'?'saturn':a.kind==='environment'&&a.id==='io-torus'?'jupiter':destination?.kind==='target'&&isMacroFamily(destination.id)?destination.id:null;
    return parent&&familyData.error.split('；').some(message=>message.startsWith(`${bodyById[parent].name}卫星：`))?`${bodyById[parent].name}卫星当前日期的历表加载失败`:'';
  };
  const topicReason=(step:TopicStep)=>{
    const missing=step.stages.filter(id=>!stageFlags[id]);
    if(missing.length)return `请开启 ${missing.map(id=>STAGES.find(s=>s.id===id)!.title).join('、')}`;
    const a=step.action;
    if(a.kind==='cosmic')return a.id==='neighbors'&&!stars.data?(stars.error||'等待恒星目录'):'';
    const satelliteError=topicSatelliteError(step);if(satelliteError)return satelliteError;
    if(a.kind==='learning')return learningReason(LEARNING_STEPS.findIndex(s=>s.id===a.id));
    if(a.kind==='boundary')return '';
    if(!scientificFrame||timeControls.loading||timeControls.error)return '等待当前日期的真实历表';
    if(a.kind==='motion'&&needsMoonFamily(a.id)&&!familyStates.some(m=>m.id==='moon'))return '等待月球当前历表';
    if(a.kind==='family'&&!familyStates.some(m=>m.parentId===a.id))return '等待该家族卫星历表';
    if(a.kind==='member'&&!macroMemberAnchor(scientificFrame,memberBatch,a.id))return '等待成员当前历表';
    if(a.kind==='detail'&&a.id==='eros')return !erosShapeReady?'等待爱神星历表':erosShapeData.error||(!erosShapeData.data?'等待爱神星形状资料':'');
    if(a.kind==='detail'&&a.id==='enceladus'&&!enceladusReady)return '等待土卫二历表';
    if(a.kind==='environment'&&a.id==='io-torus'&&!ioReady)return '等待木卫一历表';
    return '';
  };
  const topicSatelliteFailure=[...new Set((currentTopic?currentTopic.route.steps.slice(currentTopic.index,currentTopic.index+2):TOPIC_ROUTES.map(r=>r.steps[0])).map(topicSatelliteError).filter(Boolean))].join('；');
  const topicFollowing=(()=>{
    if(!currentTopic||topicReason(currentTopic.step))return false;
    const a=currentTopic.step.action;
    if(a.kind==='cosmic')return scope==='cosmic'&&cosmicId===a.id&&(a.id!=='neighbors'||stellarView===a.view&&selectedStar===STELLAR_GUIDE_HIP);
    if(scope!=='solar')return false;
    if(a.kind==='learning'){
      const lesson=LEARNING_STEPS.find(s=>s.id===a.id)!,d=lesson.destination;
      if(d.kind==='target'&&isMacroFamily(d.id)&&!effectiveLayers.moons)return false;
      if(d.kind==='zone'&&d.id!=='all'&&!effectiveLayers[d.id])return false;
      return !motionStep&&learningMatches(lesson,{scope,zone:selected,tab:solarTab,target:integratedTarget,member:selectedMember,cosmic:cosmicId,phenomenon:focusIntent});
    }
    if(a.kind==='boundary')return boundaryStep?.id===a.id;
    if(a.kind==='motion')return motionStep?.id===a.id;
    if(a.kind==='environment')return environmentStep?.id===a.id&&effectiveIntegrated[environmentStep.intent];
    if(a.kind==='family')return activeFamily===a.id&&!familySelected&&effectiveLayers.moons&&familyRings;
  ...
  const goTopic=(routeId:string,index:number)=>{
    ...
    if(a.kind!=='motion'&&timeControls.playing)timeControls.onToggle();
    setLearningIndex(null);setMotionLesson(null);setIntegratedPlaying(false);setPlanetFocus(false);setHeightScale(1);setShowPlane(false);setCameraView('oblique');setFamilySelected(null);setFamilyContext(false);setFocusIntent(null);setScope('solar');setSolarTab('zones');
    setLayers({...boundaryLayers('heliosphere'),heliosphere:false,moons:a.kind==='family'||a.kind==='detail'&&a.id==='enceladus'||a.kind==='environment'&&a.id==='io-torus'});
    setIntegratedChoices({solar:false,environment:false,belts:false,dust:false,helio:false});
    if(a.kind==='learning'){goLearning(LEARNING_STEPS.findIndex(s=>s.id===a.id));setLearningIndex(null);}
    else if(a.kind==='cosmic'){setScope('cosmic');setCosmicId(a.id);setIntegratedTarget(null);setSelectedMember(null);if(a.id==='neighbors'){setStellarView(a.view??'space');setSelectedStar(STELLAR_GUIDE_HIP);}setStellarReset(n=>n+1);}
    else if(a.kind==='boundary')chooseBoundary(a.id);
    else if(a.kind==='motion')chooseMotion(a.id);
    else if(a.kind==='environment')chooseEnvironmentStep(a.id);
    else if(a.kind==='family'){setFamilyRings(true);setFamilyOrbits(true);focusFamily(a.id);}
    else if(a.kind==='member')chooseMember(a.id);
    else if(a.id==='eros'){setErosShapeChoices(defaultErosShapeChoices());focusErosShape();}
  ...
  return <><section data-history={showHistory} aria-hidden={showHistory||undefined} style={{'--history-left':historicalViewport.left+'px','--history-top':historicalViewport.top+'px','--history-width':historicalViewport.width+'px','--history-height':historicalViewport.height+'px'} as CSSProperties} ref={dialog} className="macro-structure" role="region" aria-label="太阳系全景主页" aria-labelledby="macro-title" onKeyDown={onKeyDown} onPointerDownCapture={event=>{if(showHistory)return;captureNavigation();dialog.current?.querySelectorAll<HTMLDetailsElement>('.macro-tools-menu[open], .observation-path > details[open]').forEach(details=>{if(!details.contains(event.target as Node))details.open=false;});}} onClickCapture={event=>{if(event.detail===0)captureNavigation();}}>
    ...
    <ObservationPath onMembers={scope==='solar'?revealMembers:undefined} onTopics={revealTopics} title={scope==='cosmic'?COSMIC_LEVELS.find(l=>l.id===cosmicId)!.name:observationTitle} hint={observationHint} back={observationHistory.past.at(-1)?.title} forward={observationHistory.future.at(-1)?.title} backReason={observationBlocked(observationHistory.past.at(-1))} forwardReason={observationBlocked(observationHistory.future.at(-1))} onBack={()=>traverseHistory('back')} onForward={()=>traverseHistory('forward')} onLesson={learningIndex===null?undefined:returnLesson} learning={<LearningRoute {...learningProps} mode="map"/>}/>
        ...
        {scope==='solar'&&<section className="panorama-integration" hidden={panelTab!=='integrated'} aria-label="全景现象控制">
          ...
          <MotionLessonPanel occultationView={occultationView} onOccultationView={chooseOccultationView} coorbitalBatch={coorbitalData.batch} coorbitalEnabled={stageFlags.members} coorbitalError={coorbitalData.error} onRetryCoorbital={coorbitalData.retry} trackYear={coorbitalTracks.year} trackCount={coorbitalTracks.tracks.length} trackError={coorbitalTracks.error} onRetryTracks={coorbitalTracks.retry} familyStates={familyStates} familyError={familyData.error.split('；').find(e=>e.startsWith('木星卫星'))??''} onRetryFamily={familyData.retry} frame={scientificFrame} phasesEnabled={stageFlags.families} onSpeed={onSpeedChange} current={motionStep} epoch={motionEpoch} now={scientificFrame?.time??null} time={motionEvent?{...timeControls,start:motionEvent.start,end:motionEvent.end}:timeControls} enabled={stageFlags.structure} onLesson={chooseMotion} onExit={restorePanorama}/>
          <BoundaryComparisonPanel current={boundaryStep} enabled={stageFlags.structure&&stageFlags.heliosphereExplorer} onSelect={chooseBoundary} onReset={restorePanorama} onStages={onOpenStages}/>
          ...
          <MacroPrimaryPanel autoReveal={learningIndex===null&&!motionStep} active={motionEvent||isCoorbital(motionStep?.id)?null:primaryId(integratedTarget)} frame={scientificFrame} date={displayDate} time={timeControls} onFocus={id=>focusIntegrated(primaryTarget(id))} onFamily={focusFamily} familiesEnabled={stageFlags.families}/>
        ...
        {scope === 'solar' && <section hidden={panelTab !== 'layers'} className="macro-layer-panel" aria-label="宏观图层">
          <SceneInventory onOccultation={()=>{chooseMotion('io-occultation');setPanelTab('integrated');}} occultationStatus={!stageFlags.structure?'阶段 01 未开启':!scientificFrame||timeControls.loading||timeControls.error?'当前历表未就绪':motionStep?.id==='io-occultation'?presenceText(sceneSnapshot,'io-occultation'):'2026-01-12 地心视线 · 点选切换日期'} onSolarEclipse={()=>{chooseMotion('solar-eclipse');setPanelTab('integrated');}} solarEclipseStatus={!stageFlags.structure?'阶段 01 未开启':!scientificFrame||timeControls.loading||timeControls.error?'当前历表未就绪':motionStep?.id==='solar-eclipse'?presenceText(sceneSnapshot,'solar-eclipse'):'2026-08-12 球面月影 · 点选切换日期'} eclipseEnabled={stageFlags.structure&&!!scientificFrame&&!timeControls.loading&&!timeControls.error} onLunarEclipse={()=>{chooseMotion('lunar-eclipse');setPanelTab('integrated');}} eclipseStatus={!stageFlags.structure?'阶段 01 未开启':!scientificFrame||timeControls.loading||timeControls.error?'当前历表未就绪':motionStep?.id==='lunar-eclipse'?presenceText(sceneSnapshot,'lunar-eclipse'):'2026-03-03 事件截面 · 点选切换日期'} coorbitalStatus={!stageFlags.members?'阶段 03 未开启':coorbitalData.error||(!coorbitalData.batch?'正在读取当期历表':!effectiveLayers.planetary?'行星区域图层已隐藏':presenceText(sceneSnapshot,'coorbital'))} onCoorbital={()=>{chooseMotion('coorbital-sun');setPanelTab('integrated');}} onEnceladus={focusEnceladus} enceladusStatus={enceladusStatus} onErosShape={focusErosShape} erosShapeStatus={erosShapeStatus} onPatroclus={focusPatroclus} patroclusStatus={patroclusStatus} onEris={focusEris} erisStatus={erisStatus} onPluto={()=>{if(!stageFlags.families){onOpenStages();return;}setBinarySmall(true);focusBinary();requestAnimationFrame(()=>infoScroll.current?.querySelector('.panorama-binary')?.scrollIntoView({block:'start'}));}} plutoStatus={!stageFlags.families?'阶段 04 已隐藏':planetFocus?'专注行星中隐藏':!layers.moons?'卫星图层已关闭':plutoMoonsData.error?'历表加载失败 · 可重试':!plutoStates.length?'等待当前历表':!binarySmall?'已收起四颗小卫星':presenceText(sceneSnapshot,'pluto-small-moons')} smallRingsStatus={smallRingsStatus} onSmallRings={focusSmallRings} ringsStatus={!stageFlags.families?'阶段已隐藏':planetFocus?'专注中隐藏':!familyRings?'已关闭':!layers.planetary?'行星图层已关闭':!scientificFrame?'等待历表':presenceText(sceneSnapshot,'rings')} snapshot={sceneSnapshot} status={layerStatus} phenomenonStatus={phenomenonStatus} onLocate={locateLayer} onPhenomenon={id=>{const item=INTEGRATED_ITEMS.find(i=>i.id===id)!;if(!stageFlags[item.stage]){onOpenStages();return;}focusIntegrated(item.target,item.id);requestAnimationFrame(()=>infoScroll.current?.querySelector(`[data-panorama-phenomenon="${id}"]`)?.scrollIntoView({block:'start'}));}} onHistory={openHistory}/><h3>全景图层开关</h3><p>勾选后按当前尺度展开。靠近显示的图层保留选择，缩放时自动出现。</p>
          <div>
        ...
        {scope === 'solar' && <>          <section hidden={panelTab !== 'sources'} className="macro-source-panel"><PanoramaStarPanel data={stars.data} error={stars.error} enabled={starBackground} selected={backgroundHip} onEnabled={()=>setStarBackground(v=>!v)} onRetry={stars.retry} onInspect={hip=>{setSelectedStar(hip);setStellarView('sky');setCosmicId('neighbors');setScope('cosmic');}}/><h2>画面从哪里来</h2><p>绿色：历表位置；蓝色：观测支持的示意；紫色：模型推断。颜色用作分类，具体边界请看下方来源。</p>
    ...
    <HomeTimeControls playDisabled={!!motionEvent&&(!scientificFrame||scientificFrame.time<motionEvent.start||scientificFrame.time>=motionEvent.end)} date={displayDate} time={timeControls} onSpeed={onSpeedChange}/>
    <footer className="macro-footer"><Compass size={13}/>{scope==='solar'&&<button data-star-background-button onClick={()=>revealStarBackground()}>{stars.error?'星表未加载 · 重试':stars.data?'星表背景 · 2,936 星':'星表读取中…'}</button>}{scope==='cosmic'&&cosmicId==='neighbors'?'恒星视图固定于 J1991.25，不使用下方太阳系观测日期。空间样本按光年同比例；天球只表达方向。星点不是恒星真实尺寸。':scope === 'cosmic' ? 'AU 是太阳系内尺度；光年用于恒星和星系距离。不同镜头独立取景，画面尺寸、方位与点数不表示真实比例或实测位置；背景星点为绘制示意。' : '太阳系包含行星、卫星、矮行星、小天体、尘埃与太阳风。行星、12 个区域代表成员与三颗彗星采用历表位置；淡色椭圆是瞬时参考轨道。点云、可选双尾、风与边界是示意，奥尔特云是推断；背景采用 Hipparcos 固定星表方向（J1991.25），不是当天夜空。'}</footer>
