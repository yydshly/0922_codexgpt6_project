...
const dynamicIds = new Set<string>([...BODIES.map(body => body.id), ...SATELLITES.map(body => body.id)]);
export const OBSERVATION_COUNTS = {
  dynamic: dynamicIds.size,
  satellites: BODIES.filter(body => body.kind === 'moon').length + SATELLITES.length,
