...
const improvements = [
  { title: '统一比例双球比较（本地验收）', detail: '从全景目录打开天体大小比较，任选太阳、八大行星或月球两两对照，查看参考直径和比例。正交投影保持共同尺度，小球不单独放大；表面为静态贴图，间距与光照为排版。关闭返回原全景，补充 JPL / IAU 参数出处。' },
  { title: '全景内容目录（本地验收）', detail: '新增常驻目录，按天体与运动、空间现象、演示与阅读组织 14 个入口。跳转只滚动说明面板，不改变镜头、日期或图层；阶段未开启仍可阅读原因。支持键盘聚焦和返回介绍，窄窗口保留操作空间。' },
  { title: '专注行星与一键恢复（本地验收）', detail: '全景可暂时隐藏卫星、其他区域和现象，保留太阳、行星及已开启的环和参考线。原图层选择、镜头和时间保持不变，侧栏始终提供恢复入口；定位彗星、卫星家族或其他区域时自动退出。隐藏状态与主动关闭分别说明，不新增天体或数据。' },
  { title: '全景行星轨道与尺度环（本地验收）', detail: '八大行星新增由当前历表状态推算的参考轨道，独立控制尺度圆环和运动方向箭头，侧视可观察倾角。箭头按距离压缩映射转换方向，长度不表示速度；曲线是瞬时二体参考，不是未来历表路径。不新增天体。' },
