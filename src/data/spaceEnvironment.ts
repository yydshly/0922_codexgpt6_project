import {NEAR_EARTH_LAYERS} from './nearEarth';
...
export const ALL_ENVIRONMENT_LAYERS=[...ENVIRONMENT_LAYERS,...NEAR_EARTH_LAYERS];
export type EnvironmentLayer=typeof ALL_ENVIRONMENT_LAYERS[number]['id'];
export type EnvironmentLayers=Record<EnvironmentLayer,boolean>;
export const defaultEnvironmentLayers=():EnvironmentLayers=>Object.fromEntries(ALL_ENVIRONMENT_LAYERS.map(layer=>[layer.id,true])) as EnvironmentLayers;
...
/** Stage gates preserve the user's per-layer choices and never touch observation time. */
export function filterEnvironmentLayers(layers:EnvironmentLayers,outer:boolean,inner:boolean):EnvironmentLayers{
 return {...Object.fromEntries(ENVIRONMENT_LAYERS.map(layer=>[layer.id,layers[layer.id]&&outer])),...Object.fromEntries(NEAR_EARTH_LAYERS.map(layer=>[layer.id,layers[layer.id]&&inner]))} as EnvironmentLayers;
}
