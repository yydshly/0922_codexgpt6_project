...
export function MotionLessonPanel({familyStates,familyError,onRetryFamily,frame,phasesEnabled,current,epoch,now,time,enabled,onSpeed,onLesson,onExit}:{familyStates:readonly MacroMoon[];familyError:string;onRetryFamily:()=>void;frame:StateFrame|null;phasesEnabled:boolean;current:MotionLesson|null;epoch:number|null;now:number|null;time:MacroTimeControls;enabled:boolean;onSpeed:(speed:number)=>void;onLesson:(id:MotionLessonId)=>void;onExit:()=>void}){
 ...
 const unavailable=(id:string|undefined)=>needsMoonFamily(id)&&!phasesEnabled||id==='jupiter-resonance'&&!satellitesReady;
 ...
 <p>当前提供机制说明与历表案例；潮汐演化、共振角及长期稳定性求解尚未实现。</p></div>{!ready&&<p role="status">{!enabled?'需开启阶段 01 宏观结构。':time.error?'历表读取失败，请从时间栏重试。':time.loading?'正在读取所选日期历表…':'需要真实太阳系历表。'}</p>}{current&&<><h4>{current.title}</h4>
 {!isTheory&&<div className="motion-playback" data-motion-playback>
  ...
  <p data-motion-duration>当前 {motionSpeedLabel(time.speed)}；{isJupiter?'一个木卫三平均绕木星周期':resonance==='moon'?'一个平均绕地周期':resonance==='mercury'?'两个平均绕日周期':isPhase?'一个平均月相循环':isOrbit?'一个公转周期':'自转一圈'}约需屏幕时间 {durationLabel}（不含载入等待）。</p>
