import {PhenomenaAudit} from './PhenomenaAudit';
import {ScienceAudit} from './ScienceAudit';
...
const stages = [
  { number: '01', icon: Database, title: '从哪里来', detail: '太阳、八大行星和月球的位置与速度取自 NASA/JPL Horizons；另外 19 颗选定卫星取自 NASA/JPL NAIF 的卫星 SPK；谷神星、冥王星家族、彗星、小天体和选定双体的几何状态也取自 Horizons，各组版本与范围见下方清单。半径、质量和朝向参考 JPL/NAIF 资料。' },
  ...
  return <div className="data-journey-overlay" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}>
    <section className="data-journey-dialog" role="dialog" aria-modal="true" aria-label="太阳系数据来源与实现原理">
      ...
      <div className="data-journey-content">
        ...
        <button className="data-audit-jump" onClick={event=>event.currentTarget.closest('.data-journey-content')?.querySelector('.science-audit')?.scrollIntoView({block:'start'})}>查看本轮科学核验 ↓</button>
        <button className="data-audit-jump" onClick={event=>event.currentTarget.closest('.data-journey-content')?.querySelector('.phenomena-audit')?.scrollIntoView({block:'start'})}>画面有多真实？↓</button>
        ...
        <div className="data-journey-mode"><span className="eyebrow">TWO WAYS TO ADVANCE TIME</span><div><strong>真实太阳系</strong><p>历表给出指定时刻的几何状态，经验证的插值补足采样点之间的时间。</p></div><div><strong>物理验证</strong><p>从历表初始状态出发，用牛顿点质量引力和 300 秒固定步长推演；它是简化模型，会逐渐偏离历表。</p></div><small>当前画面：{mode === 'ephemeris' ? '真实太阳系' : '物理验证'} · 当前 {OBSERVATION_COUNTS.allDynamic} 个动态观测目标中，仅太阳、八大行星与月球进入十体引力模型；其他目标为独立观测层。</small></div>
        <div className="data-journey-boundary"><strong>怎样理解精度？</strong><p>几个数据组的留出检查点最大插值差：核心历表约 0.078 km，扩展卫星约 0.930 km，谷神星、冥王星与卡戎约 0.166 km。其他组及本轮重算范围见下方核验清单。它只衡量项目插值相对原始历表/SPK 的差，不代表历表本身的绝对位置误差，也不代表物理验证模型的误差。默认全景还会压缩距离并增强球体可见性；准确比较尺寸和距离请切换“真实比例”。</p></div>
        <ScienceAudit/>
        <PhenomenaAudit/>
