...
export function MasterPlan({onVisit,stages,initialTab='baseline',initialFilter='全部'}:{onVisit:(entry:CoverageEntry,tab:PlanTab,filter?:string)=>void;stages:StageFlags;initialTab?:PlanTab;initialFilter?:string}){
 ...
 return <div className="master-plan" ref={root}>
  ...
  {tab==='baseline'&&<><p className="master-plan-note">后续顺序已补正：内部成员 → 远缘对象 → 家族与外观 → 环境和天象 → 全景路线 → 宇宙背景 → 总验收。<button onClick={()=>setTab('route')}>查看后续九批行动 →</button></p><BaselineReview onVisit={entry=>onVisit(entry,'baseline')} onCoverage={()=>setTab('coverage')}/></> }
  {tab==='route'&&<section><ExecutionPlan onTopic={showTopic}/><details className="execution-engineering"><summary>查看对应的 7 个里程碑与 21 个工作包</summary><h3>七个里程碑 · 从基线到完整认知版</h3><p>M0–M6 保留为工程归属；实际交付采用上方 R01–R09，优先补内部成员，星空排在第八批。所有里程碑仍需各自验收，已有功能可以复用。</p><div className="master-milestones">{MASTER_PLAN.phases.map(item=><button key={item.id} aria-pressed={phase===item.id} onClick={()=>setPhase(item.id)}><b>{item.id}</b><strong>{item.title}</strong><small>{item.status}</small></button>)}</div><article className="master-phase" aria-live="polite"><span>{m.id} · {m.status}</span><h3>{m.title}</h3><p>{m.goal}</p><p><strong>前置依赖：</strong>{m.dependency}</p>{m.packages.map(w=><section key={w.id}><h4>{w.id} · {w.title}</h4><p>{w.scope}</p><p className="master-acceptance"><strong>完成标准：</strong>{w.acceptance}</p></section>)}</article><p className="master-plan-note">现有覆盖清单与验收入口已交付。下一批 R01 先核查候选数据并补近地小行星、特洛伊群与半人马族；共用供数规则和资源预算随本批推进。</p></details></section>}
 ...
 <p>{r.how}</p><p className="coverage-plan-links"><strong>后续交付：</strong>{DELIVERY_BATCHES.filter(b=>b.topics.includes(r.id)&&!['R07','R09'].includes(b.id)).map(b=>b.id).join(' / ')}。<button onClick={()=>setTab('route')}>查看九批计划</button></p>
 {SCOPE_ADDITIONS.some(s=>s.topics.includes(r.id))&&<p><strong>需补子项：</strong>{SCOPE_ADDITIONS.filter(s=>s.topics.includes(r.id)).map(s=>s.title).join('、')}（规划内容，未计入当前实现）。</p>}{r.entry?<button disabled={blocked} onClick={()=>onVisit(r.entry!,'coverage',filter)}>查看现有{r.id==='E07'?'分类':r.id==='E18'?'示意':'入口'} ↗</button>:<p>暂无观测入口</p>}
