...
const improvements = [
  { title: '第八批：行星际尘埃与流星（本地验收）', detail: '从有厚度的尘埃云和倾斜碎屑流，切换到进入大气的流星示例。支持分层开关、侧视、暂停和进度控制；区分流星体、流星与陨石，并关联真实彗星。仅为原理示意，不增加事件或历表。' },
  ...
  { title: '按阶段控制新增内容', detail: '顶部“阶段导览”按八批说明新增内容、画面含义和观察顺序；支持独立开关、只看一批和恢复全部阶段。01–03 筛选宏观页，04 控制新增环系与家族讲解，05 控制磁层与极光，06 控制辐射带与等离子体层，07 控制太阳活动演示，08 控制尘埃与流星演示。' },
  { title: '四大巨行星的环与卫星', detail: '接通木星、土星、天王星、海王星家族，补三颗巨行星的暗淡环，保留土星环。细环宽度增强可关闭；来源、代表环段及未绘制范围明确列出。这一批没有新增卫星数量。' },
  { title: '宏观优化已发布', detail: '第一至第七批已随 f266de0 完成部署，提交正文包含功能、来源边界和验证。第八批仅在本地验收，尚未推送。' },
  ...
  return <div className="product-progress-overlay" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}>
    <section className="product-progress-dialog" role="dialog" aria-modal="true" aria-labelledby="product-progress-title">
      ...
      <div className="product-progress-content">
        ...
        <section className="product-progress-next"><h3>验收时重点看什么？</h3><p>先打开“阶段导览 → 08 → 只看这一批”。侧看尘埃厚度与倾斜碎屑流，再切换进入大气、播放或拖动到发光和消融，最后接着看真实彗星。主页暂停的日期应保持不变；两个镜头独立缩放，不是实测颗粒轨迹。探测器待定，地月课程后移。</p><p>真实比例下天体非常小是物理尺度的结果。查看参数与数据清单时，注意球体外观、瞬时参考轨道、真实历表位置和十体物理模型的不同来源。</p></section>
