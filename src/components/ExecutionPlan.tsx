...
export function ExecutionPlan({onTopic}:{onTopic:(id:string)=>void}){
 const [selected,setSelected]=useState('R09');
 ...
 <p className="master-plan-note">R01–R08 已有对应上线能力；R09 已有科学核验与恢复修复，实机及用户总验收尚未完成。当前剩余动作见“收尾进度”C01–C05，本页保留原批次要求，不把发布当作验收通过。</p>
 <h3>九批交付归属 · 当前状态</h3>
 <p>{EXECUTION_VERSION} · 各批状态分别记录。原 M0–M6 的编号保留，下面按可验收版本安排交付。</p>
 <p className="master-plan-note">这九批是原定内容交付顺序；主页“阶段导览”的九批开关控制已有画面。两者互不对应。选择下面的批次只查看计划，不会开启场景或增加天体。</p>
 <a href={executionUrl} download="太阳系九批交付与范围补正.md">下载九批动作与验收标准 ↗</a>
 <ol className="execution-sequence">{DELIVERY_BATCHES.map((b,i)=><li key={b.id}><button aria-pressed={selected===b.id} aria-controls="execution-detail" onClick={()=>setSelected(b.id)}><b>{String(i+1).padStart(2,'0')}</b><span>{b.title}<small>{b.id==='R09'?'收尾中 · 总验收未完成':b.id==='R08'?'已上线 · F01 未关闭':'已上线 · 待用户验收'}</small></span></button></li>)}</ol>
 ...
 <h3>补正后的首版范围</h3><p>20 类是产品目录，不能据此宣称穷尽全部天文学分类。下列补项跨目录关联，既有功能可复用；S01–S05 已有代表实现与证据，逐类用户验收仍待完成。</p>
