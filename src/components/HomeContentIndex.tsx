...
export function HomeContentIndex({stages,ready,onVisit,onStages,children}:{children?:ReactNode;stages:StageFlags;ready:number;onVisit:(item:CoverageItem)=>void;onStages:()=>void}){
 return <section className="home-content-index" aria-label="全景接入总表"><h2>已有内容都在哪里？</h2>
 <p>已收录 {OBSERVATION_COUNTS.allDynamic} 个动态目标；当前可定位 {ready} 个。可定位数量不代表此刻可见数量：阶段开关、日期加载和观察尺度都会影响显示。</p>
