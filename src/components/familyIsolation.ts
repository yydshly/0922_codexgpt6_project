...
export function isolateFamilyContext(scene:THREE.Scene,parent:PrimaryId,saved:Map<THREE.Object3D,boolean>){
 for(const object of scene.children){
  if(object instanceof THREE.Light||object.userData.background||object.name==='macro-real-families'||object.userData.primaryId===parent)continue;
...
export function isolateEnvironmentContext(scene:THREE.Scene,parent:'sun'|'earth'|'jupiter'|'sun-earth',saved:Map<THREE.Object3D,boolean>){
 for(const object of scene.children){
