...
function MacroCanvas({ historical,onPresence,active, historyBridge, integrated, selectedMember, onSelectMember, focusRequest, restoreRequest, cometBatch, cometTracks, showActivity, frame, selected, resetCount, family, cameraView, heightScale, showPlane, layers, batch, animate, showLabels, onDistance }: { historical:HistoricalView|null;onPresence:(snapshot:SceneSnapshot)=>void;active:boolean; historyBridge:RefObject<CameraHistoryBridge>; integrated:IntegratedOptions; selectedMember:string|null; onSelectMember:(id:string)=>void; focusRequest:number; restoreRequest:number; cometBatch: StateBatch | null; cometTracks: CometTracks | null; showActivity: boolean; frame: StateFrame | null; selected: MacroZoneId; resetCount: number; family?: SolarFamilyId; cameraView: MacroCameraView; heightScale: 1 | 10; showPlane: boolean; layers: MacroLayerVisibility; batch: StateBatch | null; animate: boolean; showLabels: boolean; onDistance: (distance: number) => void }) {
  ...
  useEffect(() => {
    ...
    const draw = (now: number) => {
      ...
      const motion=integration.motion;const orbitalLesson=motion==='earth-orbit'||motion==='mercury-resonance';const orbitalBody=motion==='mercury-resonance'?'mercury':'earth';
      ...
      if(orbitalLesson)isolateEnvironmentContext(scene,orbitalBody==='mercury'?'sun-mercury':'sun-earth',familyHidden,true);
  ...
  return <div className="macro-canvas" ref={host} role="group" aria-label="可拖动旋转、滚轮缩放的太阳系宏观结构三维示意">{integrated.motion==='moon-phase'&&<MoonPhaseReadout compact frame={frame}/>}{integrated.motion==='earth-seasons'&&<SeasonReadout compact frame={frame}/>}{integrated.motion&&spinOrbitBody(integrated.motion)&&<SpinOrbitReadout compact body={spinOrbitBody(integrated.motion)!} now={frame?.time??null} epoch={integrated.motionEpoch}/>}{integrated.motion==='jupiter-resonance'&&<OrbitalResonanceReadout compact now={frame?.time??null} epoch={integrated.motionEpoch} states={integrated.families.states}/>}</div>;
}
...
export function MacroStructure({ active=true,onSpeedChange,onHomeTool,initialPanel,initialCosmic,returnLabel,stageFlags, onOpenStages, onOpenEnvironment, onOpenSolarActivity, onOpenDust, onOpenHeliosphere, initialZone, onOpenFamily, initialFamily, initialMemberId, timeControls, frame, displayDate, isEphemeris = true, onClose, onOpenReadingGuide, onObservePlanets, onExploreObject }: Props) {
  ...
  const motionStep=stageFlags.structure&&(!needsMoonFamily(motionLesson)||stageFlags.families&&effectiveLayers.moons)?motionLessonForView(motionLesson,integratedTarget):null;
  const chooseMotion=(id:MotionLessonId)=>{if(id==='jupiter-resonance'&&!resonanceReady(familyStates)||!stageFlags.structure||needsMoonFamily(id)&&!stageFlags.families||!scientificFrame||timeControls.loading||timeControls.error)return;const lesson=MOTION_LESSONS.find(l=>l.id===id)!;onSpeedChange(motionPlaybackSpeed(id));if(timeControls.playing)timeControls.onToggle();setIntegratedPlaying(false);setLearningIndex(null);setScope('solar');setHeightScale(1);setShowPlane(false);setFamilyContext(false);setLayers({...boundaryLayers('heliosphere'),heliosphere:false,moons:needsMoonFamily(id)});if(needsMoonFamily(id)){setFamilyOrbits(true);setFamilyRings(false);setFamilyEnhanced(false);}setIntegratedChoices({solar:false,environment:false,belts:false,dust:false,helio:false});focusIntegrated(lesson.target,null);setMotionLesson(id);setMotionEpoch(scientificFrame.time);};
  ...
  const observationBlocked=(entry:ObservationBookmark|undefined)=>{
    if(!entry)return '';
    const target=entry.target;if(needsMoonFamily(entry.motion)&&!stageFlags.families)return '请开启阶段 04 卫星家族';if(entry.motion&&!stageFlags.structure)return '请开启阶段 01 运动课程';
