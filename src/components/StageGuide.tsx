...
export function StageGuide({flags,onChange,onVisit,onClose}:{flags:StageFlags;onChange:(flags:StageFlags)=>void;onVisit:(id:StageId,only:boolean)=>void;onClose:()=>void}){
 ...
 <p>01–03 控制宏观页新增内容，04 控制新增环系与家族讲解，05 控制磁层与极光，06 控制辐射带与等离子体层；05–06 可在同一原理图中组合；07 控制独立的太阳活动演示；08 控制尘埃分布与流星过程演示；09 展开日球层与星际空间，独立于第一批宏观轮廓。05–09 也控制综合全景里的对应现象；在主页“综合全景 → 全景现象”可同场景定位，下面“前往本阶段”继续进入独立详解。主观测页既有天体、带区与土星环保留。关闭阶段优先于图层开关；重新开启会沿用原图层设置，不修改日期或物理模型。</p>
