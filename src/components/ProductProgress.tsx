  ...
  return <div className="product-progress-overlay" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}>
    <section className="product-progress-dialog" role="dialog" aria-modal="true" aria-labelledby="product-progress-title">
      ...
      <div className="product-progress-content">
        ...
        <section className="product-progress-next" data-validation-summary data-release="2026.09.25-r09.5"><h3>本次交付：2026.09.25-r09.5</h3><p>C02 第一组：修复规划入口只到区域、未定位实际对象的问题；太阳、地球、地月与土星环可直接进入并显示对应说明。合并重复画面提示，纠正行星近景显隐文案，暂隐单体近景边缘被裁切的卫星。E01–E06 的入口与代表交互已复验，完整子项和其余 19 项继续检查。</p><a href={coreAuditUrl} download="C02-CORE-COVERAGE-AUDIT.md">查看本轮检查、截图与剩余范围</a><p>总规划默认打开“收尾进度”：固定五步、20 类主题与 5 项必补核对表、4 项未关闭事项。已上线、既有验证和用户验收分别记录；本轮不增加天体或宣称整体完成。</p><p>修正旧规划把准卫星、运动课程和真实星表写成待实现的问题。网页与四份规划文档共用台账，构建时检查同步；旧记录归档，个人旧版验收存储保留。</p><a href={closeoutNotesUrl} download="首版收尾与25项核对表.md">下载五步收尾与25项核对表</a><br/><a href={textureLoadingNotesUrl} download="TEXTURE-LOADING.md">查看外观素材恢复</a><br/><a href={ephemerisLoadingNotesUrl} download="EPHEMERIS-LOADING.md">查看历表恢复</a><br/><a href={loadingNotesUrl} download="LOADING-OPTIMIZATION.md">查看窗口加载优化</a><br/><a href={releaseNotesUrl} download="RELEASE-2026-09-25.md">查看完整功能交付范围</a></section>
