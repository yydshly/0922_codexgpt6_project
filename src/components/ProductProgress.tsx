...
const improvements = [
  ...
  { title: '按阶段控制新增内容', detail: '顶部“阶段导览”按五批说明新增内容、画面含义和观察顺序；支持独立开关、只看一批和恢复全部阶段。01–03 筛选宏观页，04 控制新增环系与家族讲解，05 控制磁层原理图入口。' },
  { title: '四大巨行星的环与卫星', detail: '接通木星、土星、天王星、海王星家族，补三颗巨行星的暗淡环，保留土星环。细环宽度增强可关闭；来源、代表环段及未绘制范围明确列出。这一批没有新增卫星数量。' },
  { title: '宏观优化已发布', detail: '远端已发布 82f8515：两颗彗星、区域成员、阶段导览、巨行星环系与地球磁层均已上线。' },
  ...
  return <div className="product-progress-overlay" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}>
    <section className="product-progress-dialog" role="dialog" aria-modal="true" aria-labelledby="product-progress-title">
      ...
      <div className="product-progress-content">
        ...
        <section><div className="product-progress-section-heading"><Clock3 size={16}/><h3>近期更新 · 已发布</h3></div><ol className="product-progress-steps">{improvements.map(item => <li key={item.title}><strong>{item.title}</strong><p>{item.detail}</p></li>)}</ol></section>
        <section className="product-progress-next"><h3>验收时重点看什么？</h3><p>先打开“阶段导览”，点第五批的“只看这一批”。分别开关五层结构，侧面看磁尾、靠近看极光，再返回原观测。暂停的日期应保持不变；主页本来在播放则继续推进。也可以进入真实地球近景，再通过“磁层与极光”返回。探测器待定，地月课程后移。</p><p>真实比例下天体非常小是物理尺度的结果。查看参数与数据清单时，注意球体外观、瞬时参考轨道、真实历表位置和十体物理模型的不同来源。</p></section>
