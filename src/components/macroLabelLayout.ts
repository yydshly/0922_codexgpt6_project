export interface LabelAnchor { id: string; x: number; y: number; width: number; height: number; radius?: number }
...
export function placeMacroLabels(anchors: LabelAnchor[], width: number, height: number, top = 90, bottom = 110, obstacles: {x:number; y:number; width:number; height:number}[] = []): LabelBox[] {
  ...
  for (const a of anchors) {
    ...
    const side=Math.max(16,(a.radius??0)+gap),above=Math.max(12,(a.radius??0)+gap),below=Math.max(14,(a.radius??0)+gap);
    const positions = [[a.x+side,a.y-a.height-above],[a.x-a.width-side,a.y-a.height-above],[a.x+side,a.y+below],[a.x-a.width-side,a.y+below],[a.x-a.width/2,a.y-a.height-Math.max(36,above)]];
