...
export function MotionLessonPanel({frame,phasesEnabled,current,epoch,now,time,enabled,onSpeed,onLesson,onExit}:{frame:StateFrame|null;phasesEnabled:boolean;current:MotionLesson|null;epoch:number|null;now:number|null;time:MacroTimeControls;enabled:boolean;onSpeed:(speed:number)=>void;onLesson:(id:MotionLessonId)=>void;onExit:()=>void}){
 ...
 return <section className="environment-journey" data-motion-lessons aria-label="自转与公转课程"><h3>运动课程：自转、公转、月相与季节</h3><p>先看球体自身转动，再拉远看绕日运行，再对照金星，接着理解月相和地球季节。连续播放与单步对照共用观测日期，整幅太阳系同步更新。选择课程后已准备好推荐速度，点击「连续播放」即可观察。</p><ol>{MOTION_LESSONS.map((s,i)=><li key={s.id}><button disabled={!ready||s.id==='moon-phase'&&!phasesEnabled} aria-pressed={s.id===current?.id} onClick={()=>onLesson(s.id)}>{i+1} · {s.name}</button></li>)}</ol>{!phasesEnabled&&<p>月相课程还需开启阶段 04 卫星家族。</p>}{!ready&&<p role="status">{!enabled?'需开启阶段 01 宏观结构。':time.error?'历表读取失败，请从时间栏重试。':time.loading?'正在读取所选日期历表…':'需要真实太阳系历表。'}</p>}{current&&<><h4>{current.title}</h4>
 <div className="motion-playback" data-motion-playback>
  ...
  <p data-motion-duration>当前 {motionSpeedLabel(time.speed)}；{isPhase?'一个平均月相循环':isOrbit?'一个公转周期':'自转一圈'}约需屏幕时间 {durationLabel}（不含载入等待）。</p>
 ...
 <p>{current.text}</p><p className="environment-legend">{current.note}</p>{isSeason?<SeasonReadout frame={frame}/>:isPhase?<><MoonPhaseReadout frame={frame}/><p>绕地球一周约 27.3 天；从一次新月到下一次平均约 29.5 天。后者仅用于估计观看时长，画面位置仍由历表决定。</p></>:<p>本体模型恒星自转周期：{(spinPeriodSeconds(current.body)/3600).toLocaleString('zh-CN',{maximumFractionDigits:3})} 小时；资料公转周期：{bodyById[current.body].orbitalPeriodDays.toLocaleString('zh-CN',{maximumFractionDigits:3})} 日。</p>}<h4>单步对照 · 暂停并跳转日期</h4><div className="environment-controls"><button disabled={!can(-delta)} onClick={()=>move(-delta)}>{isPhase?'后退 3 天':isOrbit?'后退 30 天':'后退 ¼ 自转周期'}</button><button disabled={!can(delta)} onClick={()=>move(delta)}>{isPhase?'前进 3 天':isOrbit?'前进 30 天':'前进 ¼ 自转周期'}</button><button disabled={!ready||epoch===null||epoch<time.start||epoch>time.end} onClick={()=>{if(epoch!==null){if(time.playing)time.onToggle();time.onSeek(epoch);}}}>重置本节日期</button></div><p>每次步进 {Math.round(delta/3600*100)/100} 小时；越出内置历表范围时禁用。单步直接显示新日期，不播放中间过程；连续运动请使用上方播放按钮。课程与底部倍率同步，快进自转可能产生采样错觉。退出会暂停，保留当前日期和倍率；需要返回起点时先重置本节日期。</p><div className="environment-controls"><button disabled={!ready||index===0} onClick={()=>onLesson(MOTION_LESSONS[index-1].id)}>上一节运动</button><button disabled={!ready||index===MOTION_LESSONS.length-1||MOTION_LESSONS[index+1]?.id==='moon-phase'&&!phasesEnabled} onClick={()=>onLesson(MOTION_LESSONS[index+1].id)}>下一节运动</button></div><button onClick={()=>{pause();onExit();}}>退出运动课并恢复全景</button><p><a href={current.source} target="_blank" rel="noreferrer">NASA 概念解释 ↗</a> · <a href={PHYSICAL_SOURCE} target="_blank" rel="noreferrer">JPL 参数 ↗</a> · <a href="https://naif.jpl.nasa.gov/pub/naif/generic_kernels/pck/pck00011.tpc" target="_blank" rel="noreferrer">NAIF 自转常数 ↗</a></p>{isSeason?<small>两半球对照采用既有地轴姿态近似与同日历表；四个日期用于观察变化，未求解精确分至点。<a href="https://www.weather.gov/fgz/Seasons" target="_blank" rel="noreferrer">NWS：理想昼长与实测日出日落的区别 ↗</a></small>:isPhase?<small>主画面的月球亮面取决于当前自由镜头，旁边的月相圆面固定地心参照，所以二者不必相同。太阳在局部画面外，箭头只示传播方向；地月距离压缩，不能用此图判断食的几何条件。</small>:<small>金色线和粉色标记为教学标注，标记不是地理点。自转使用原有 NAIF 线性角速度与静态极轴近似，绝对经度/姿态不用于测绘；本体周期由该角速度反算，可能与四舍五入的参数表略有差别。</small>}</>}{!current&&<p>选择课程同步定位；原有「行星运动 · 谁转得快」仍提供所有行星的参数比较。</p>}</section>;
}
