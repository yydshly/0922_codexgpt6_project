...
export function ObservationPath({title,hint,back,forward,backReason,forwardReason,onBack,onForward,learning,onLesson,onTopics,onMembers}:{title:string;hint:string;back?:string;forward?:string;backReason:string;forwardReason:string;onBack:()=>void;onForward:()=>void;learning:ReactNode;onLesson?:()=>void;onTopics?:()=>void;onMembers?:()=>void}){
 return <nav className="observation-path" aria-label="全景观察路径">
  ...
  {onMembers&&<button onClick={onMembers}>成员清单</button>}
  {onTopics&&<button onClick={onTopics}>专题路线</button>}
