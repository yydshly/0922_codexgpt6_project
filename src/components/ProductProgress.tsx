  ...
  return <div className="product-progress-overlay" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}>
    <section className="product-progress-dialog" role="dialog" aria-modal="true" aria-labelledby="product-progress-title">
      ...
      <div className="product-progress-content">
        ...
        <section className="product-progress-next" data-validation-summary data-release="2026.09.25-r09.2"><h3>本次交付：2026.09.25-r09.2</h3><p>历表目录提前请求；主历表、卫星和扩展天体每个文件最多等待 30 秒。失败可原地重试所选日期，恢复后保持暂停。底部时间栏显示等待阶段，画面标明上次有效时刻。</p><p>321 项自动检查通过，并完成请求卡住、月份失败、原日期恢复与迟到请求隔离检查。主包体积提示、真实设备体验和用户总验收仍待处理。</p><a href={ephemerisLoadingNotesUrl} download="EPHEMERIS-LOADING.md">下载历表加载与恢复说明</a><br/><a href={loadingNotesUrl} download="LOADING-OPTIMIZATION.md">查看上一批窗口加载优化</a><br/><a href={releaseNotesUrl} download="RELEASE-2026-09-25.md">查看完整功能交付范围</a></section>
