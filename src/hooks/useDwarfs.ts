...
export function useDwarfs(time: number | undefined, enabled: boolean) {
  ...
  const result = useMemo(() => {
    ...
    try { return { batch: dwarfProvider.sample(time), error: '' }; }
    catch (error) { return { batch: null, error: error instanceof Error ? error.message : '矮行星历表读取失败' }; }
  }, [time, enabled, revision]);

  useEffect(() => {
    if (!enabled || time === undefined || result.batch || result.error || failure?.key === key) return;
    const token = ++generation.current;
    void dwarfProvider.get(time).then(() => {
      if (token === generation.current && current.current.enabled && current.current.key === key)
        setRevision(value => value + 1);
    }).catch(error => {
      if (token === generation.current && current.current.enabled && current.current.key === key)
        setFailure({ key, message: error instanceof Error ? error.message : '矮行星历表读取失败' });
    });
    return () => { if (token === generation.current) generation.current++; };
  }, [enabled, key, result.batch, result.error, failure]);
