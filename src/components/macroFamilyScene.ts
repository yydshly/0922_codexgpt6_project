...
import {MACRO_FAMILIES,familyLocalPosition,familyPlanetRadius,type MacroFamilyId,type MacroMoon} from '../data/macroFamilies';
import {satelliteParentAttitude,satelliteOrbitPoints} from './SatelliteSystem';
...
export function createMacroFamilies(scene:THREE.Scene,host:HTMLElement,onSelect:(id:string)=>void,onFocus:(id:MacroFamilyId)=>void){
 ...
 const moons=new Map<string,{mesh:THREE.Mesh<THREE.SphereGeometry,THREE.MeshStandardMaterial>;orbit:THREE.Line;parent:MacroFamilyId;epoch:number}>();
 ...
 return {
  ...
  update(frame:StateFrame|null,options:FamilySceneOptions,camera:THREE.PerspectiveCamera,target:MacroFamilyId|null,anchors:Record<string,THREE.Vector3>){
   ...
   for(const state of options.states){
    ...
    if(!object){const r=familyPlanetRadius(state.parentId)*.9*Math.pow(state.radiusKm/bodyById[state.parentId].radiusKm,.36);const mesh=new THREE.Mesh(new THREE.SphereGeometry(r,24,18),new THREE.MeshStandardMaterial({color:state.color,roughness:1}));mesh.userData.familyMoon=state.id;const orbit=new THREE.Line(new THREE.BufferGeometry(),new THREE.LineBasicMaterial({color:'#8cbcbf',transparent:true,opacity:.3,depthWrite:false}));f.group.add(mesh,orbit);object={mesh,orbit,parent:state.parentId,epoch:NaN};moons.set(state.id,object);makeLabel(state.id,state.name,()=>onSelect(state.id));}
    object.mesh.position.set(...familyLocalPosition(state.position,state.parentId));object.mesh.visible=options.moons;object.mesh.material.emissive.set(state.id===options.selected?'#436354':'#000000');
